...
class Pets extends Model
{
	...
	public function dog($request)
	{
		...
		$brd        = $request->brd ?? null;
		// $size       = $request->size ?? 0;
		$age        = $request->age ?? null;
		$dogAge     = $request->dogAge ?? null;
		$dogBreed   = $request->dogBreed ?? null;
		$a          = $request->a ?? null;
		$b          = $request->b ?? null;
		$c          = $request->c ?? null;
		if ($operations == "1") {
			if ($brd == "1") {
				...
				$size = $request->size ?? "1"; // Default to size 1 if not provided

				if ($age > 0 && $age < 20) {
					$y = explode(".", $age);
					if (count($y) == "1") {
						$dogi_age = $kuty[$age];
						$answer = $dogi_age[$size - 1]; // Arrays are 0-indexed
					} else if (count($y) == "2") {
						$point = "0." . $y[1];
						$percent = $point * 100;
						$chuk = $y[0];
						...
						$show_baz1 = $dogi_age1[$size - 1];
						$show_baz2 = $dogi_age2[$size - 1];
						...
						if ($b >= 1) {
							if ($c >= 1) {
								...
								$breeds = [
									...
									['Miniature Schnauzer', 12, 15, 'mininature_schnauzer', 'Ratting, Terrier Group', 'Childrens', 'Moderate', '12-15 years', '30-36 cm', '5.4-9.1 kg', 'Intelligent, Fearless, Obedient, Friendly', 'Yes', 'Black, Salt & Pepper, White'],
	...
	{
		$extra_lenght = $request->extra_lenght;
	...
	public function puppy($request)
	{
		...
		$weight_selection = trim($request->weight_selection);
		$age = trim($request->age);
		$age_selection = trim($request->age_selection);
		$type = $request->type ?? null;
		$select_breed = $request->select_breed ?? null;
	...
	public function pearson($request)
	{
		...
		if (!empty($dob) && !empty($date)) {
			...
			$this->param['birth_month'] = $birth_month;
			$this->param['N_r_months'] = $next_r_mon;
			$this->param['N_r_days'] = $next_r_day;
			$this->param['Years'] = $Totalyears;
			$this->param['Months'] = $Total_months;
			$this->param['Weeks'] = $Total_weeks;
			$this->param['Days'] = $Total_days;
			$this->param['Hours'] = $Total_hours;
			$this->param['Min'] = $Total_minuts;
			$this->param['Sec'] = $Total_seconds;
			$this->param['RESULT'] = 1;
			// dd($this->param);
			return $this->param;
		} else {
	...
	public function cat_age($request)
	{
		$year = $request->year;
		$month = $request->month;
