...
class HomeController extends Controller
{
	...
	public function category($category)
	{
		...
		if ($category == 'health') {
			...
			$des = "It's health that is wealth! Many people struggle with health issues. You can use our health calculator to monitor your health, track your progress, and make decisions about healthcare. The tools provide accurate insights into your health metrics. Before relying solely on the results, it's important to consult with a medical expert. Take the first step towards improving your health by trying our calculator today.";
			...
			$des = 'People are not good at mathematics. Our collection of advanced math calculators makes it fun to do math. The collection includes Geometry, Calculus, Algebra, Trigonometry, and Arithmetic. These tools make math easy to understand for students and professionals.';
			...
			$des = 'Our physics calculator can help you solve problems related to mechanics, thermodynamics, electromagnetism, and other physical phenomena. In both academic and practical settings, use the tools to support your physical analysis. Start using the physics calculator today.';
			...
			$des = 'The subject of chemistry is very interesting. To make it even more engaging, we have developed different tools, which can help resolve chemistry problems instantly. The calculator can help you understand chemical reactions and properties. Make your chemistry more comprehensible with the help of the calculator below.';
			...
			$des = "Anyone who is struggling with statistical calculations can use our statistics calculator. Statistics help you solve problems quickly and accurately. You'll find practical solutions and gain valuable insight into the world of statistics. If you want to make your data interpretation more authentic, start using the advanced statistical calculators below.";
			...
			$des = 'Financial planning is important for both seasoned entrepreneurs and students. A comprehensive suite of tools can be found in our collection of calculators. From business planning and investments to tax calculations and debt management, we have got you covered. It is easy to set and achieve your financial goals with our calculator.';
		} elseif ($category == 'everyday-life') {
			$meta_title = 'Everyday Life Calculators - Free Informational Tools and Calculators';
			$meta_des = 'The accomplished team of calculator-logical.com provided a wide range of interesting and Everyday Life calculators that you can utilize on a daily basis.';
			$des = 'Make daily life more convenient and efficient with the collection of everyday life calculators that offer assistance with various tasks. From unit conversions to cooking time calculations, fuel efficiency, tip calculation, and distance calculation, these calculator can help solve everyday problems. By saving time, improving accuracy, planning effectively, and simplifying tasks, everyday life calculator can help update your daily routines.';
		} elseif ($category == 'construction') {
			$meta_title = 'Construction Calculators';
			$meta_des = 'Our Construction Calculators can be used to simplify your construction projects. Access accurate and easy-to-use tools to plan and execute your construction tasks efficiently, from material estimations to project cost calculations.';
			$des = 'Construction calculators are used by professionals and do-it-yourselfers in construction projects. Accurate calculations for different materials and costs can be made with our variety of tools. There are many calculators on this page, including ones for volume, material quantity, and construction projects.';
		} elseif ($category == 'pets') {
			$meta_title = 'Pets Calculators';
			$meta_des = 'Our Pets Calculators are designed to help you care for your pets. Easy-to-use tools tailored for pet owners are available.';
			$des = 'The tools in this list are designed to assist pet owners. Pet health, feeding, and other factors can be calculated and estimated by them. The tools below can help you make informed decisions about your furry friends.';
		...
		if (isset($cal_data)) {
			if (isset($request->submit)) {
				...
				if ($url == 'ideal-gas-law-calculator') {
					...
					$fun = 'weeks_ago';
				} elseif ($url == 'hours-ago-calculator') {
					$fun = 'time_ago';
				} elseif ($url == 'years-ago-calculator') {
					$fun = 'year_ago';
				} elseif ($url == 'days-ago-calculator') {
					$fun = 'days_ago';
				} elseif ($url == 'days-left-in-the-year') {
					$fun = 'days_left';
				} elseif ($url == 'julians-date-calculator') {
					$fun = 'julians';
				} elseif ($url == 'weeks-between-dates-calculator') {
					$fun = 'weeks_between';
				} elseif ($url == 'months-from-now') {
					$fun = 'months_from';
				} elseif ($url == 'weeks-left-in-the-year') {
					$fun = 'weeks_left';
				} elseif ($url == 'months-left-in-the-year') {
					$fun = 'months_left';
				} elseif ($url == 'how-many-days-until-my-birthday') {
					$fun = 'birthday_days';
				} elseif ($url == 'days-since-date-calculator') {
					$fun = 'days_since';
				} elseif ($url == 'ai-answer-generator') {
					$fun = 'ai_answer';
				} elseif ($url == 'how-many-pages-calculator') {
					$fun = 'page_count';
				} elseif ($url == 'how-many-words-calculator') {
					$fun = 'word_count';
				}
			...
			if (isset($cal_data->is_calculator)) {
				if ($cal_data->is_calculator == 'Calculator') {
					...
					// dd($componentPath, ucwords($data['page']));
				...
				if (!class_exists($componentPath)) {
					...
					// dd($sub_cn,$where_con,$all_sub);
					// $all_sub = DB::table('calculators')->where('cal_cat',$dfsd)->select('cal_link')->get();
	...
	{
		// dd($lang,$category,$url);

			// $all_converter = DB::table('calculators')->select('cal_cat', 'cal_title', 'is_calculator', 'is_same', 'cal_link')->where('cal_link', $category . '/' . $url)->first();

			// $where = array('cal_cat' => $all_converter->cal_cat, 'is_calculator' => 'Sub-Converter');
	...
	{
		$converters = DB::table('calculators')->select('cal_link', 'cal_title', 'cal_cat', 'no_index')->where('is_calculator', 'Converter')->get();
		$all_converters = DB::table('calculators')->select('cal_cat', 'cal_title', 'is_calculator', 'is_same', 'cal_link')->where('is_calculator', 'Sub-Converter')->get()->toArray();
		$counting = DB::table('calculators')->select('cal_cat', 'cal_title', 'is_calculator', 'is_same', 'cal_link')->where('is_calculator', 'Sub-Converter')->get()->count();
		// Group sub-converters by category
		$grouped_converters = [];
		foreach ($all_converters as $converter) {
			$grouped_converters[$converter->cal_cat][] = $converter;
		}
		// dd($converters);
		// dd($grouped_converters[$converters]);

		// Pass data to view
		$data['converters'] = $converters;
		$data['counting'] = $counting;
		$data['grouped_converters'] = $grouped_converters;
		$data['converters'] = $converters;
		$data['allcategories'] = $this->allcategories;
		$data['device'] = $this->device;

		$data['meta_title'] = 'Conversion Calculator - Metric Units Measurement Converter';
		$data['meta_des'] = "The unit conversion calculator for metric/imperial units convert";
		return view('pages/sitemap-two', $data);
	}
