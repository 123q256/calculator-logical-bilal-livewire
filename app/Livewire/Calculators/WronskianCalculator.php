...
class WronskianCalculator extends Component
{
    public $EnterEq = '(x^2+4), sin(2x), cos(x)';
    public $with = 'x';
    ...
    {
        $this->EnterEq = '(x^2+4), sin(2x), cos(x)';
        $this->with = 'x';
    ...
    public function calculate()
    {
        ...
        if (empty($this->with)) {
            $this->error = 'The variable W.R.T is required.';
            session()->flash('validation_error', $this->error);
            $this->detail = null;
            return;
        }
        ...
        $model = new Math();
        $result = $model->wronskian($request);

        if ($result === false) {
            $this->error = 'The number of functions cannot be less than 2 or greater than 5.';
            session()->flash('validation_error', $this->error);
            $this->detail = null;
            return;
        }
    ...
    public function render()
    {
        ...
        return view('livewire.calculators.wronskian-calculator');
    }
}
