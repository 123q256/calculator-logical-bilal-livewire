@if ($detail)
    <div class="flex flex-nowrap px-4 py-3 mb-1 bg-gradient-to-r from-blue-700 to-blue-500 border border-gray-200 shadow-sm rounded-xl items-center mt-2 justify-between gap-2 w-full" id="calculator-result-actions-container">
        <!-- Feedback Section -->
        <div id="actual_feedback" class="flex-grow flex justify-start">
            <div class="flex flex-col items-start text-left">
                <div id="feedback-initial-state" class="flex flex-col sm:flex-row sm:items-center">
                    <strong class="text-[15px] text-white font-semibold mb-1.5 sm:mb-0 sm:mr-3">Was this helpful?</strong>
                    <div class="bg-[#F5F5F5] rounded-md flex items-center text-[14px] px-2 py-1 shadow-sm border border-gray-200 w-max">
                        <button type="button" class="cursor-pointer flex items-center px-2 py-0.5 feedback-btn hover:text-blue-600 transition-colors" data-type="like" data-calculator-name="{{ $calculatorName ?? '' }}" data-calculator-link="{{ !empty($calculatorLink) ? $calculatorLink : Request::path() }}">
                            <span class="mr-1">😊</span> <span>Yes</span>
                        </button>

                        <div class="mx-1 text-gray-300">|</div>

                        <button type="button" class="cursor-pointer flex items-center px-2 py-0.5 feedback-btn hover:text-blue-600 transition-colors" data-type="dislike" data-calculator-name="{{ $calculatorName ?? '' }}" data-calculator-link="{{ !empty($calculatorLink) ? $calculatorLink : Request::path() }}">
                            <span class="mr-1">😔</span> <span>No</span>
                        </button>
                    </div>
                </div>

                <div id="feedback-loading-state" class="hidden">
        ...
        <div class="flex items-center justify-end gap-2 shrink-0">
            <button type="button" id="openResultShareBtn"
                class="w-10 h-10 bg-[#F5F5F5] rounded-lg cursor-pointer hover:bg-gray-200 flex items-center justify-center transition-colors">
                <img width="16" alt="Share" src="{{ asset('assets/new-share-CJ_G3xQj.png') }}">
            </button>
            <button type="button"
                class="open-feedback-btn w-10 h-10 bg-[#F5F5F5] rounded-lg cursor-pointer hover:bg-gray-200 flex items-center justify-center transition-colors">
                <img width="18" alt="Report" src="{{ asset('assets/report-DErXaa7-.png') }}">
            </button>
        </div>
    </div>

    <!-- Share Modal -->
    <div id="resultShareModalOverlay" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center hidden" style="z-index: 99;">
        <div class="relative bg-white rounded-lg shadow-lg p-6 max-w-md w-full animate-fadeIn" id="resultShareModalContent">
            <!-- Close Button -->
            <button id="closeResultShareBtn"
            ...
            <p class="text-xl font-bold text-center mb-4 text-black">Share the {{ $calculatorName ?? 'Calculator' }}</p>
            <!-- Social Media Icons -->
            @php
                $encodedUrl = urlencode($pageUrl ?? url()->current());
                $shareLinks = [
            ...
            <!-- Copyable Link -->
            <div class="flex items-center bg-gray-100 p-3 rounded-lg relative">
                <input id="resultShareUrlInput" value="{{ urldecode($encodedUrl) }}"
                    class="flex-grow bg-transparent border-none input_border text-gray-600 px-4" readonly>
                <button id="copyResultShareLinkBtn" class="ml-2 text-white rounded-lg px-2 py-1 focus:outline-none">
                    <img src="{{ asset('assets/images/copy.svg') }}" alt="Copy">
                </button>
                <!-- Toast -->
                <div id="resultCopyToast"
                    class="absolute -top-12 right-0 bg-black text-white px-4 py-2 rounded-md text-sm shadow-lg z-50 hidden">
                    ✅ Link copied!
                </div>
            </div>
        </div>
