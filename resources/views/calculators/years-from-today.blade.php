...
@push('calculatorJS')
    <script>
         ...
         const year = new Date();
        const DaysContainer = document.getElementById('yearsContainer');
        const twoDaysContainer = document.getElementById('twoyearsContainer');
        ...
        for (let i = 1; i <= 25; i++) {
            const addedYearsDate = addYears(year, i);
            const formattedDateS = formatDate(addedYearsDate);
            const formattedDate = formatDateString(addedYearsDate);
            const dateName = getDayName(formattedDateS);

            const DayElement = document.createElement('tr');
            DayElement.innerHTML = `
                <tr>
                   <td class="bg-gray"><time datetime="P${i}Y">${i} years</time></td>
                    <td>
        ...
        for (let i = 26; i <= 50; i++) {
            ...
            `;
            twoDaysContainer.appendChild(DayElement);
        }

        const fiveYearsDate = addYears(year, 5);
        document.querySelectorAll('.fiveYearsDate').forEach(element => {
            element.innerHTML = formatDateString(fiveYearsDate);
        });
