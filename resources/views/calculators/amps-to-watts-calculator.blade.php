                ...
                <div class="flex flex-wrap items-center bg-blue-100 border border-blue-500 text-center rounded-lg px-1">
                    ...
                    <div class="lg:w-1/2 w-full px-2 py-1">
                        <div class="bg-white px-3 py-2 cursor-pointer rounded-md transition-colors duration-300  hover_tags hover:text-white metric" id="metric">
                            <a href="{{url('watts-to-amps-calculator')}}/" class="cursor-pointer veloTabs  text-decoration-none"><strong>{{ isset($lang['2']) ? $lang['2'] : 'Watts to Amps' }}</strong></a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="grid grid-cols-1  lg:grid-cols-2 md:grid-cols-2 mt-3  gap-2">
                ...
                <div class="col-12 px-2">
                    ...
                    <div class="relative w-full mt-[7px]">
                       <input type="number" name="current" id="current" step="any"  class="border border-gray-300 p-2 rounded-lg focus:ring-2  w-full" value="{{ isset($_POST['current'])?$_POST['current']:'20' }}" aria-label="input" placeholder="00" oninput="checkInput()"/>
                       <label for="current_unit" class="absolute cursor-pointer text-sm underline right-6 top-4" onclick="toggleDropdown('current_unit_dropdown')">{{ isset($_POST['current_unit'])?$_POST['current_unit']:'mW' }} ▾</label>
                       <input type="text" name="current_unit" value="{{ isset($_POST['current_unit'])?$_POST['current_unit']:'mW' }}" id="current_unit" class="hidden">
                ...
                <div class="col-12 px-2">
                    ...
                    <div class="relative w-full mt-[7px]">
                       <input type="number" name="voltage" id="voltage" step="any"  class="border border-gray-300 p-2 rounded-lg focus:ring-2  w-full" value="{{ isset($_POST['voltage'])?$_POST['voltage']:'20' }}" aria-label="input" placeholder="00" oninput="checkInput()"/>
                       ...
                       <div id="voltage_unit_dropdown" class="absolute z-10 bg-white border border-gray-300 rounded-md w-auto mt-1 right-0 hidden" to="voltage_unit">
                           <p class="p-2 hover:bg-gray-100 cursor-pointer" onclick="setUnit('voltage_unit', 'mA')">mA</p>
                           <p class="p-2 hover:bg-gray-100 cursor-pointer" onclick="setUnit('voltage_unit', 'A')">A</p>
                           <p class="p-2 hover:bg-gray-100 cursor-pointer" onclick="setUnit('voltage_unit', 'kA')">kA</p>
                       </div>
                    </div>
                </div>
                <div class="col-12 px-2 hidden" id="power_factor_div">
                    ...
                    <div class="w-100 py-2 position-relative">
                        <input type="number" type="any" name="power" max="1" id="power" class="input" value="{{ isset($_POST['power'])?$_POST['power']:'1' }}" aria-label="input" placeholder="00" />
                    </div>
                </div>
            </div>
       </div>
                    ...
                    <div class="w-full">
                        <div class="w-full md:w-[60%] lg:w-[60%] mt-2">
                            <table class="w-100 font-s-18">
                                <tr>
                                    <td class="py-2 border-b">{{ $lang['16'] }} ({{ $lang['17'] }})</td>
                                    <td class="py-2 border-b"><strong class="text-blue">{{round($detail['power_ans'], 2)}} W</strong></td>
                                </tr>
                                <tr>
                                    <td class="py-2 border-b">{{ $lang['16'] }} ({{ $lang['18'] }})</td>
                                    <td class="py-2 border-b"><strong class="text-blue">{{($detail['power_ans'] / 1000)}} kW</strong></td>
                                </tr>
                                <tr>
                                    <td class="py-2 border-b">{{ $lang['16'] }} ({{ $lang['19'] }})</td>
                                    <td class="py-2 border-b"><strong class="text-blue">{{number_format($detail['power_ans'] * 1000)}} mW</strong></td>
                                </tr>
                            </table>
                        </div>
                    </div>
