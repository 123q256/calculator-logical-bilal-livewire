    ...
    <div class="w-full mx-auto p-4 lg:p-8 md:p-8 input_form rounded-lg  space-y-6 my-3">
       ...
       <div class="lg:w-[60%] md:w-[60%] w-full mx-auto ">
            ...
            <div class="col-span-12">
                <label for="EnterEq" class="label"><?=$lang['1']?> f(x,y): <?=$lang['2']?> e^x <?=$lang['3']?> e^{x}</label>
                <div class="w-full py-2 relative">
                    <input type="text" name="EnterEq" id="EnterEq" class="input" value="{{ isset($request->EnterEq)?$request->EnterEq:'2x^2 + e^{y} - 3xy^2' }}" aria-label="input"/>
            ...
            <div class="col-span-12">
                <label for="with" class="label"><?=$lang['4']?> W.R.T:</label>
            ...
            <div class="col-span-12 hidden">
                <label for="how" class="label">{{$lang['5']}}:</label>
                <div class="w-full py-2">
                    <select name="how" class="input" id="how" aria-label="select">
                        <option value="1">1</option>
                        <option value="2" {{ isset($request->how) && $request->how=='2'?'selected':'' }}>2</option>
                        <option value="3" {{ isset($request->how) && $request->how=='3'?'selected':'' }}>3</option>
                        <option value="4" {{ isset($request->how) && $request->how=='4'?'selected':'' }}>4</option>
                        <option value="5" {{ isset($request->how) && $request->how=='5'?'selected':'' }}>5</option>
                    </select>
                </div>
            </div>
        </div>
                    ...
                    <div class="w-full text-[16px]">
                        @if($how > 1)
                            ...
                            @for($i=1; $i < count($detail['final_res']); $i++)
                                <p class="mt-3text-[18px]"><strong>\( f{{$j}} (x)\) {{$lang['7']}}</strong></p>
                                <p class="mt-3text-[18px]">\( {{$detail['final_res'][$i+1]}} \)</p>
                                @php $i++;$j.="'"; @endphp
                                @php $i++; @endphp
                                <div class="w-full mt-3">
                                    <button type="button" class="calculate repeat" style="font-size: 16px;padding: 10px;cursor: pointer;" chk="{{$i}}">{{$lang['8']}}</button>
                                </div>
                            ...
                            @endfor
                        @else
                            <p class="mt-3text-[18px]"><strong>{{$lang['7']}}</strong></p>
                            <p class="mt-3text-[18px]">\( {{$detail['simple']}} \)</p>
                            <p class="mt-3text-[18px]"><strong>Solution:</strong></p>
