        ...
        <div class="lg:w-[60%] md:w-[60%] w-full mx-auto ">
            <div class="grid grid-cols-1  lg:grid-cols-2 md:grid-cols-2  gap-4">
                <div class="space-y-2">
                    ...
                    <div class="relative w-full ">
                        <input type="number" name="age" id="age" step="any"  class="border border-gray-300 p-2 rounded-lg focus:ring-2  w-full" value="{{ isset($_POST['age'])?$_POST['age']:'5' }}" aria-label="input" placeholder="00" oninput="checkInput()"/>
                        <label for="age_selection" class="absolute cursor-pointer text-sm underline right-6 top-4" onclick="toggleDropdown('age_selection_dropdown')">{{ isset($_POST['age_selection'])?$_POST['age_selection']:'wks' }} ▾</label>
                        <input type="text" name="age_selection" value="{{ isset($_POST['age_selection'])?$_POST['age_selection']:'wks' }}" id="age_selection" class="hidden">
                        <div id="age_selection_dropdown" class="absolute z-10 bg-white border border-gray-300 rounded-md lg:w-[40%] md:w-[40%] w-[40%] mt-1 right-0 hidden">
                            <p class="p-1 hover:bg-gray-100 cursor-pointer" onclick="setUnit('age_selection', 'days')">{{ $lang['2'] }}</p>
                            <p class="p-1 hover:bg-gray-100 cursor-pointer" onclick="setUnit('age_selection', 'wks')">{{ $lang['3'] }}</p>
                            <p class="p-1 hover:bg-gray-100 cursor-pointer" onclick="setUnit('age_selection', 'mon')">{{ $lang['4'] }}</p>
                        </div>
                    </div>
                 </div>
                 <div class="space-y-2">
                    ...
                    <div class="relative w-full ">
                        ...
                        <label for="weight_selection" class="absolute cursor-pointer text-sm underline right-6 top-4" onclick="toggleDropdown('weight_selection_dropdown')">{{ isset($_POST['weight_selection'])?$_POST['weight_selection']:'kg' }} ▾</label>
                        <input type="text" name="weight_selection" value="{{ isset($_POST['weight_selection'])?$_POST['weight_selection']:'kg' }}" id="weight_selection" class="hidden">
                        <div id="weight_selection_dropdown" class="absolute z-10 bg-white border border-gray-300 rounded-md lg:w-[25%] md:w-[25%] w-[33%] mt-1 right-0 hidden">
                            @foreach (["g", "dag", "kg", "oz", "lb", "stone"] as $name)
                            <p class="p-1 hover:bg-gray-100 cursor-pointer" onclick="setUnit('weight_selection', '{{$name}}')">{{$name}}</p>
                            @endforeach
                        </div>
                    </div>
                 </div>
            ...
            <div class="grid grid-cols-1 gap-4">
                <div class="inline-block cursor-pointer current_gpa flex items-center my-3">
                    ...
                    <p class="ml-auto transition-transform transform button">▼</p>
                </div>
            </div>

            <div class="grid grid-cols-1   gap-4">
                <div class="current_input hidden overflow-hidden transition-all duration-500 ease-in-out">
                    <label for="select_breed" class="font-s-14 text-blue">{!!$lang['17']!!}:</label>
                    <select class="input" name="select_breed" id="select_breed">
                        <option value="Affenpinscher" {{ isset($_POST['select_breed']) && $_POST['select_breed'] === 'Affenpinscher' ? 'selected' : '' }}>Affenpinscher</option>
                    ...
                    <div class="w-full  p-3 rounded-lg mt-3">
                        <div class="my-2">
                                ...
                                <table class="w-full">
                                    <tr>
                                        <td class="border-r-2 pr-4">
                                            <strong class="text-4xl text-green">{{ round($detail['puppy1'], 2) }}</strong> 
                                            <span class="text-lg text-blue">(Kg)</span>
                                        </td>
                                        <td class="pl-4">
                                            <strong class="text-4xl text-green">{{ round($detail['puppy2'], 2) }}</strong>
                                            <span class="text-lg text-blue">(lb)</span>
                                        </td>
                                    </tr>
                                </table>
                                <p class="my-3">{{ $lang['7'] }}: <strong>{{ round($detail['cal4'], 3) }}</strong> - <strong>{{ round($detail['cal3'], 3) . " kg" }}</strong></p>
                            </div>
                            ...
                            <div class="mt-4 lg:mt-2">
                                <p class="text-lg font-semibold">{{ $lang['13'] }} {{ $detail['select_breed'] }} {{ $lang['14'] }}</p>
                                @if ($detail['names_one'] == "1")
                                <p class="text-2xl my-2 font-semibold text-blue">{{ $detail['names_zero'] }}</p>
                                @else
                                <p class="text-2xl my-2 font-semibold text-blue">{{ $lang['15'] }}: {{ $detail['names_zero'] }}</p>
                                <p class="text-2xl my-2 font-semibold text-blue">{{ $lang['16'] }}: {{ $detail['names_one'] }}</p>
                                @endif
                            </div>
                            @endif
                        </div>
