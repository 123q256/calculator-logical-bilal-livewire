                ...
                <div class="w-full p-3 rounded-lg mt-3 overflow-auto">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div class="p-1">
                            <div class="border rounded-lg bg-white p-2">
                                <p>📅 Weeks</p>
                                <p class="text-lg font-semibold"><strong>{{ $detail['currentWeekOfYear'] }} / {{ $detail['weeksInYear'] }}</strong></p>
                            </div>
                        </div>
                        <div class="p-1">
                            <div class="border rounded-lg bg-white p-2">
                                <p>📅 Year</p>
                                <p class="text-lg font-semibold"><strong>{{ $detail['currentDayOfYear'] }} / {{ $detail['daysInYear'] }}</strong></p>
                            </div>
                        </div>
                    </div>
                    <p class="text-center p-2">
                        What is <strong>{{ $request->number }}</strong> weeks ago? The answer is <strong>{{ $detail['date_name'] }}</strong>, <strong>{{ $detail['t_date'] }}</strong>. It is the week <strong>{{ $detail['currentWeekOfYear'] }}</strong> of the total <strong>{{ $detail['weeksInYear'] }}</strong> weeks of the year. It also marks the day <strong>{{ $detail['currentDayOfYear'] }}</strong> out of <strong>{{ $detail['daysInYear'] }}</strong> days of the year.
                    </p>
                </div>
            </div>
        </div>
        ...
        function getDayName(date, locale) {
            ...
            for (let i = 1; i <= 50; i++) {
                ...
                weekElement.innerHTML = `
                    <tr>
                        <td class="bg-gray"><time datetime="P${i}W">${i} Weeks ago from today</time></td>
                        <td>
            ...
            // 12 week
            document.getElementById('addtwlWeeks').innerHTML = formatDateString(addtwlWeeks);
            // 4 week
            document.getElementById('addFourWeeks').innerHTML = formatDateString(addFourWeeks);
            // 6 week
            document.getElementById('addSixWeeks').innerHTML = formatDateString(addSixWeeks);
            // 8 week
            document.getElementById('addEightWeeks').innerHTML = formatDateString(addEightWeeks);
            // 10 week
            document.getElementById('addTenWeeks').innerHTML = formatDateString(addTenWeeks);
    </script>
@endpush