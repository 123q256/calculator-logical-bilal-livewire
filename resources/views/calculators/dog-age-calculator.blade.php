       ...
       <div class="lg:w-[60%] md:w-[60%] w-full mx-auto ">
            <div class="grid grid-cols-12   gap-2 md:gap-4 lg:gap-4">
                <div class="col-span-12">
                    ...
                    <input type="radio" name="operations" id="bedtime" value="1"  {{ isset($_POST['operations']) && $_POST['operations'] === '1' ? 'checked' : '' }}>
                    <label for="bedtime" class="label pe-lg-3 pe-2">{{ $lang['2'] }}:</label>
                    <input type="radio" name="operations" id="wkup" value="2" checked {{ isset($_POST['operations']) && $_POST['operations'] === '2' ? 'checked' : '' }}>
                    <label for="wkup" class="label">{{ $lang['3'] }}:</label>
                    {{-- <input type="hidden" name="operations" id="operations" value="1"> --}}
                </div>
                <div class="col-span-12 md:col-span-6 lg:col-span-6 simple_vr hidden">
                    <label for="breeds" class="label">{{ $lang['4'] }}</label>
                    <div class="w-full py-2"> 
                        <select name="brd" id="breeds" class="input">
                            @php
                            ...
                            @php
                                ...
                                $name = [$lang['5'], $lang['6'],];
                                $val = ['1', '2',];
                                optionsList($val,$name,isset($_POST['breeds'])?$_POST['breeds']: '1' );
                            @endphp
                        </select>
                    </div>
                </div>
                <div class="col-span-12 md:col-span-6 lg:col-span-6 simple_vr hidden">
                    ...
                    <div class="w-full py-2 relative"> 
                        <input type="number" step="any" name="age" id="age" class="input" aria-label="input"  value="{{ isset($_POST['age'])?$_POST['age']:'4' }}" />
                    </div>
                </div>
                <div class="col-span-12 md:col-span-6 lg:col-span-6 ad_vr ">
                    <label for="dogAge" class="label">{{ $lang['11'] }}</label>
                    <div class="w-full py-2"> 
                        <select class="input" id="dogAge" name="dogAge">
                ...
                <div class="col-span-12 md:col-span-6 lg:col-span-6 ad_vr ">
                    ...
                    <div class="w-full py-2"> 
                        <select class="input" id="dogBreed" name="dogBreed">
                            ...
                            <option value="9&&American Staffordshire Terrier">American Staffordshire Terrier</option>
                            ...
                            <option value="45&&Cavalier King Charles Spaniel">Cavalier King Charles Spaniel</option>
                            ...
                            <option value="90&&Irish Soft-coated Wheaten Terrier">Irish Soft-coated Wheaten Terrier</option>
                            ...
                            <option value="119&&Nova Scotia Duck Tolling Retriever">Nova Scotia Duck Tolling Retriever' 12</option>
                            ...
                            <option value="126&&Petit Basset Griffon Vendeen">Petit Basset Griffon Vendeen</option>
                            ...
                            <option value="170&&West Highland White Terrier">West Highland White Terrier</option>
                            <option value="171&&Whippet">Whippet</option>
                            <option value="172&&Wire Hair Fox Terrier">Wire Hair Fox Terrier</option>
                            <option value="173&&Wirehaired Pointing Griffon">Wirehaired Pointing Griffon</option>
                ...
                <div class="col-span-12 md:col-span-6 lg:col-span-6 ad_vr ">
                    ...
                    <div class="w-full py-2 relative"> 
                        <input type="number" step="any" name="a" id="a" class="input" aria-label="input"  value="{{ isset($_POST['a'])?$_POST['a']:'4' }}" />
                    </div>
                </div>
                <div class="col-span-12 md:col-span-6 lg:col-span-6 ad_vr ">
                    <label for="b" class="label">{{ $lang['18'] }}</label>
                    <div class="w-full py-2 relative"> 
                        <input type="number" step="any" name="b" id="b" class="input" aria-label="input"  value="{{ isset($_POST['b'])?$_POST['b']:'4' }}" />
                    </div>
                </div>
                <div class="col-span-12 md:col-span-6 lg:col-span-6 ad_vr ">
                    <label for="c" class="label">{{ $lang['19'] }}</label>
                    <div class="w-full py-2 relative"> 
                        <input type="number" step="any" name="c" id="c" class="input" aria-label="input"  value="{{ isset($_POST['c'])?$_POST['c']:'4' }}" />
                    </div>
                </div>
                        ...
                        <div class="row my-2">
                            @if ($operations === '1')
                                <div class="text-center">
                                    <p class="my-3 text-[18px]">{{$lang['33']}} <strong class="text-[32px] bg-sky px-3 py-2 rounded-lg text-blue-500">{{round($detail['answer'])}} </strong> {{$lang['34']}}</p>
                                </div>    
                            @else
                                <div class="w-full md:w-[80%] lg:w-[80%] flex justify-between">
                                    <div>
                                        <p class="pe-lg-5">In human years, your dog is <strong class="text-green-500 text-[21px]"> {{number_format($detail['dogHumanAge'], 1)}}  </strong>  years old and<br> is considerd <strong class="text-green-500 text-[21px]">{{$detail['type']}}</strong></p>
                                    </div>
                                    <div>
                                        <img src="{{asset('images/dogs/'.$detail['images'].'.png')}}" id="im" class="max-image" alt="Dog Images" width="140px" height="140px">
                                    </div>
                                </div>
                                <div class="w-full md:w-[80%] lg:w-[80%] text-[18px]">
                                    <p class="text-[20px] mt-2"><strong>{{ $detail['name']." Details"}}</strong></p>
                                    <table class="w-full">
                                        <tr>
...
@push('calculatorJS')
    <script>
        var simple_vr = document.querySelectorAll('.simple_vr');
        var ad_vr = document.querySelectorAll('.ad_vr');
        @if(isset($detail) || isset($error))
            var bedtime = document.getElementById('bedtime');
            var wkup = document.getElementById('wkup');
            if(bedtime.checked){
                ad_vr.forEach(element => {
                    element.style.display = "none";
                });
                simple_vr.forEach(element => {
