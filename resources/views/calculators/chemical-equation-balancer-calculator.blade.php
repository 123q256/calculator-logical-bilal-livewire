       ...
       <div class="lg:w-[60%] md:w-[60%] w-full mx-auto ">
            ...
            <div class="col-span-12 md:col-span-6 lg:col-span-6">
                <div class="d-flex align-items-end justify-content-between">
                    ...
                    <button type="button" class="bt_set  bg-[#2845F5] text-white" id="load_example">{!! $lang['2'] !!}</button>
                </div>
                <div class="w-100 py-2 position-relative">
                    <input type="text" step="any" name="eq" id="eq" class="input" aria-label="input" placeholder="00" value="{{ isset(request()->eq)?request()->eq:'Fe + O2 = Fe2O3' }}" autocomplete="off" />
                </div>
            ...
            <div class="col-span-12 md:col-span-6 lg:col-span-6">
                <h4>{{$lang['examp']}}</h4>
                <ul class="text-[14px] list-disc ps-4">
                    <li class="mt-2"><span class="eq_link cursor-pointer text-blue-500 underline">H2 + O2 = H2O</span></li>
                    <li class="mt-2"><span class="eq_link cursor-pointer text-blue-500 underline">CH4 + O2 = CO2 + H2O</span></li>
                    <li class="mt-2"><span class="eq_link cursor-pointer text-blue-500 underline">Mg + HCl = MgCl2 + H2</span></li>
                    <li class="mt-2"><span class="eq_link cursor-pointer text-blue-500 underline">C6H12O6 + O2 = CO2 + H2O</span></li>
                    <li class="mt-2"><span class="eq_link cursor-pointer text-blue-500 underline">NH3 + O2 = NO + H2O</span></li>
                </ul>
            </div>
            ...
            <div class="rounded-lg  flex items-center justify-center">
                <div class="w-full mt-3">
                    <div class="w-full overflow-auto">
                        <p class="text-center"><strong>{!! $lang['5'] !!}</strong></p>
                        <p class="text-center my-1">{!! $detail['inp'] !!}</p>
                        <div class='result' id='resid'>
                            ...
                            <div class="col-12 col-lg-9 mx-auto mt-2 lg:w-[60%] w-full">
                                <input type="hidden" name="type" value="stoichiometry" id="type">
                                <div class="flex flex-wrap items-center bg-blue-100 border border-blue-500 text-center rounded-lg px-1">
                                    <div class="lg:w-1/2 w-full px-2 py-1">
                                        <div class="bg-white px-3 py-2 cursor-pointer rounded-md transition-colors duration-300  hover_tags hover:text-white tagsUnit tab" id="imperial" data-value="stoichiometry">
                                            Reaction Stoichiometry
                                        </div>
                                    </div>
                                    <div class="lg:w-1/2 w-full px-2 py-1">
                                        <div class="bg-white px-3 py-2 cursor-pointer rounded-md transition-colors duration-300  hover_tags hover:text-white tab" id="metric" data-value="limiting">
                                            Limiting Reagent
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <code id='result overflow-auto'></code>
                            <div class='w-full table mt-3 overflow-auto'>
    ...
    @push('calculatorJS')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.addEventListener('click', function(event) {
                    if (event.target.classList.contains('eq_link')) {
        ...
        @if(isset($detail))
            ...
            <script>
                ...
                reactarr = [];
                prodarr = [];
                function convertReactant(reactant) {
                    return reactant.replace(/\(([^)]+)\)(\d*)/g, (match, group1, group2) => {
                ...
                for (var k=0;k<rlen;k++) {          //loop checks for the all Reactants
                    ...
                    if (reactant[k].includes('(') || reactant[k].includes(')')) {
                        var converted = convertReactant(reactant[k]);
                    }else{
                        var converted = reactant[k];
                    }
                    var ans=converted.split(/(?=[A-Z])/); //splits each reactant as seperate chemicals
                    ab = ans;
                ...
                for (var k=0;k<plen;k++) {          //loop checks for the all Reactants
                    var mmass = 0; var t_mass=0;
                    if (product[k].includes('(') || product[k].includes(')')) {
                        var converted = convertReactant(product[k]);
                    }else{
                        var converted = product[k];
                    }
                    var ans=converted.split(/(?=[A-Z])/);      //splits each product as seperate chemicals
                    ab = ans;
                    for(var i=0;i<ab.length;i++){
                    var regex = /\d+/g;
                    var string = ab[i];
                ...
                val += "<tr><th class='border-b p-2'>{!! $lang['13'] !!}</th> <th class='border-b p-2'>{!! $lang['14'] !!}</th> <th class='border-b p-2'>{!! $lang['15'] !!}</th> <th class='border-b p-2'>{!! $lang['16'] !!}(g/mol)</th>  <th class='border-b p-2'>{!! $lang['17'] !!}(g)</th> </tr> ";
                val += " <tr><th class='border-b p-2' colspan=5>{!! $lang['11'] !!}</th></tr>";
                for (i = 1; i <= rlen; i++) {
                    val += "<tr><td class='border-b p-2'>" + reactant[i-1] + "</td> <td class='border-b p-2'>" + reactratio[i-1] + "</td> <td class='border-b p-2'>" + parseFloat(reactarr[i-1].toFixed(2)) + "</td> <td class='border-b p-2'><input type='text' class='input molesInput' id='mr" + i + "' value='' onkeyup='new_calc(this)' data-coeff='" + reactratio[i-1] + "' data-mass='" + parseFloat(reactarr[i-1].toFixed(2)) + "'></td> <td class='border-b p-2'><input type='text' class='input weightInput' id='wr" + i + "' value='' onkeyup='new_calc(this)' data-coeff='" + reactratio[i-1] + "' data-mass='" + parseFloat(reactarr[i-1].toFixed(2)) + "'></td></tr>";
                }
                val+=" <tr><th class='border-b p-2' colspan=5>{!! $lang['12'] !!} </th></tr>";
                for (j = 1; j <= plen; j++) {
                    val += "<tr> <td class='border-b p-2'>" + product[j-1] + "</td> <td class='border-b p-2'>" + prodratio[j-1] + "</td> <td class='border-b p-2'>" + parseFloat(prodarr[j-1].toFixed(2)) + "</td> <td class='border-b p-2'><input type='text' class='input molesInput' id='mp" + j + "' value='' onkeyup='new_calc(this)' data-coeff='" + prodratio[j-1] + "' data-mass='" + parseFloat(prodarr[j-1].toFixed(2)) + "'></td> <td class='border-b p-2'><input type='text' class='input weightInput' id='wp" + j + "' onkeyup='new_calc(this)' data-coeff='" + prodratio[j-1] + "' data-mass='" + parseFloat(prodarr[j-1].toFixed(2)) + "'></td></tr>";
                }
                ...
                document.querySelectorAll('.tab').forEach(function(tab) {
                    tab.addEventListener('click', function() {
                        ...
                        const count_input = plen; // Number of inputs to disable or enable
                        const targetmolesDis = document.querySelectorAll('.molesInput');
                        const targetweightDis = document.querySelectorAll('.weightInput');

                        function setInputsDisabled(targetInputs, shouldDisable) {
                            const totalInputs = targetInputs.length;
                            for (let i = totalInputs - count_input; i < totalInputs; i++) {
                                if (targetInputs[i]) {
                                    targetInputs[i].disabled = shouldDisable;
                                }
                            }
                        }

                        if (val === "limiting") {
                            setInputsDisabled(targetmolesDis, true);
                            setInputsDisabled(targetweightDis, true);
                        } else {
                            setInputsDisabled(targetmolesDis, false);
                            setInputsDisabled(targetweightDis, false);
                        }
                    });
                });

                function new_calc(element){
                    var typeValue = document.getElementById('type').value;
                    if (typeValue === "limiting") {
                        var enter_class = element.classList;
                        var enter_coeff = element.getAttribute('data-coeff');
                        var enter_mass = element.getAttribute('data-mass');
                        var enter_value = element.value;
                        function updateWeightInput(moleInput) {
                            var mole_value = moleInput.value;
                        ...
                        function updateMoleInput(weightInput) {
                            var weight_value = weightInput.value;
                        ...
                        if(enter_id == ("mr"+rlen) || enter_id == ("wr"+rlen)){
                            ...
                            var enter_coeff = element.getAttribute('data-coeff');
                            var enter_mass = element.getAttribute('data-mass');
                            var enter_value = parseFloat(element.value); // Make sure to parse the value as a float
                            ...
                            if (enter_class.contains('molesInput')) {
                                ...
                                updateLastThreeInputs('.molesInput', mole_check, enter_mass, enter_coeff);
                            } else if (enter_class.contains('weightInput')) {
                                ...
                                updateLastThreeInputs('.weightInput', mole_check, enter_mass, enter_coeff);
                            }
                            function updateLastThreeInputs(selector, mole_check, mass, coeff) {
                                var inputs = Array.from(document.querySelectorAll(selector));
                                var lastThreeInputs = inputs.slice(rlen); // Get the last three inputs

                                lastThreeInputs.forEach((input) => {
                                    if (selector === '.molesInput') {
                                        var moles_coeff = input.getAttribute('data-coeff');
                                        ...
                                        correspondingWeightInput.value = parseFloat((react_moles_ans * input.getAttribute('data-mass')).toFixed(6));
                                    } else if (selector === '.weightInput') {
                                        var weightValue = parseFloat(input.value);
                                        var moles_coeff = input.getAttribute('data-coeff');
                                        var react_moles_ans = moles_coeff * mole_check;

                                        // Update the corresponding mole input
                        ...
                        if (enter_class.contains('molesInput')){
                            ...
                            var weightInput = element.closest('tr').querySelector('.weightInput');
                            weightInput.value = parseFloat((enter_value * enter_mass).toFixed(6));
                            document.querySelectorAll('.molesInput').forEach((moleInput, index) => {
                                if (element !== moleInput) {
                                    var moles_coeff = moleInput.getAttribute('data-coeff');
                                    var moles_mass = moleInput.getAttribute('data-mass');
                                    var react_moles_ans = moles_coeff * mole_check;
                            ...
                            document.querySelectorAll('.weightInput').forEach((weightInput) => {
                                if (element !== weightInput) {
                                    var moles_coeff = weightInput.getAttribute('data-coeff');
                                    var moles_mass = weightInput.getAttribute('data-mass');
                                    var react_moles_ans = moles_coeff * mole_check;
                ...
                function calc(obj){         //moles, weight and limiting calculation here
                    ...
                    for (var k=1;k<=n;k++) {      
                        if (minimum == mole[k-1]) {   // minimum value to match the reactant weight
                            ac = reactratio[k-1];
                            lr = reactant[k-1];
                            document.getElementById("opp").value="{!! $lang['13'] !!} "+lr;
                        }
                    }
                ...
                function eqn_parse() {
                    var input ="{!! $detail['chemical_equation'] !!}";
