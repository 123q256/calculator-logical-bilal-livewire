<form class="row" action="{{ url()->current() }}/" method="POST">
       ...
       <div class="lg:w-[40%] md:w-[40%] w-full mx-auto ">
            <div class="grid grid-cols-1   gap-4">
                <div class="space-y-2 relative">
                    <label for="e_date" class="font-s-14 text-blue">{!! $lang['1'] !!}:</label> 
                    <input type="date" name="e_date" id="e_date" class="input" value="{{ isset($_POST['e_date'])?$_POST['e_date']:'' }}" aria-label="input" placeholder="mm/dd/yyyy" />
                </div>
            </div>
       </div>
        @if ($type == 'calculator')
                ...
                <div class="rounded-lg  flex items-center justify-center">
                    <div class="w-full  lg:p-6 md:p-6 px-2 rounded-lg mt-6">
                        <div class="w-full text-center mt-4">
                            ...
                            <p class="lg:text-3xl md:text-3xl text-md my-3 bg-[#2845F5] text-white rounded-lg px-4 py-2 font-bold">{{ $detail['date'] }}</p>
    ...
    @unless(isset($detail))
        @push('calculatorJS')
            <script>
                    document.getElementById("e_date").defaultValue = "2022-08-07"
            </script>
        @endpush
    @endunless
</form>