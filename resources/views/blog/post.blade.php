...
@section('content')
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-8  sm:pt-5 pt-6 pb-14">
        ...
        <a href="javascript:void(0)" class="inline-flex items-center gap-2  font-medium text-lg ">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
            </svg> Blogs/{{ $post->post_cat }}
        </a>
        <h1 id="blogTitle" class="text-lg sm:text-2xl lg:text-2xl font-bold text-neutral-950 my-4">{{ $post->post_title }}</h1>
        
        <div class="flex flex-col lg:flex-row gap-8 lg:gap-12">
            <!-- Main Content -->
            <article class="flex-1 min-w-0">
                <div class="aspect-video  rounded-2xl overflow-hidden bg-gray-200 mb-6">
                    <img id="blogImage" src="{{ url('images/' . $post->post_img) }}" alt="{{ $post->post_title }}"
                        class="w-full h-full object-cover">
                </div>
                <p id="blogDate" class="text-blue-600 text-sm font-medium">{{ \Carbon\Carbon::parse($post->post_time)->format('d F Y') }}</p>
    ...
    <!-- Share Modal -->
    <div id="shareModalOverlay" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[999999] hidden">
        <div id="shareModalBox" class="relative bg-white rounded-lg shadow-lg p-6 max-w-md w-full animate-fadeIn">
            <!-- Close Button -->
            ...
            <div class="flex justify-center space-x-4 mb-6">
                <button onclick="shareSocial('facebook')"
                ...
                <button onclick="shareSocial('twitter')"
                ...
                <button onclick="shareSocial('linkedin')"
                ...
                <button onclick="shareSocial('email')"
            ...
            <div class="flex items-center bg-gray-100 p-3 rounded-lg relative">
                <input id="shareUrlInput" value="{{ url()->current() }}"
                    class="flex-grow bg-transparent border-none input_border text-gray-600 px-4" readonly>
                <button id="copyShareLinkBtn" class="ml-2 text-white rounded-lg px-2 py-1 focus:outline-none">
                    <img src="{{ asset('assets/images/copy.svg') }}" alt="Copy">
                </button>
                <!-- Toast -->
                <div id="copyToast"
                    class="absolute -top-12 right-0 bg-black text-white px-4 py-2 rounded-md text-sm shadow-lg z-50 hidden transition-opacity">
                    ✅ Link copied!
                </div>
            </div>
        </div>
