...
<div class="container-fluid mx-auto  container-fluid mt-[20px]">
    <div class="w-full max-w-7xl mx-auto py-1 rounded-lg text-center">
        <h1 class="text-2xl lg:text-3xl md:text-3xl px-5 font-semibold "><strong >All Converters</strong></h1>
    </div>
                ...
                <div class="lg:w-[100%]  md:w-[100%] w-full  order-2 lg:order-1 p-[20px] lg:p-[10px] md:p-[10px]   rounded-lg inset-0  bg-center bg-no-repeat filter" style="background-image: url('{{ asset('new_page/finance_bg.svg') }}');">
                    <div class="grid grid-cols-12 gap-4   Everyday-Life" id="myTable">
                        ...
                        @foreach($converters as $converter)
                            ...
                            @if(isset($grouped_converters[$converter->cal_title]))
                                <h2 class="mb-3 mt-1 text-[25px] col-span-12">
                                    <strong>
                                        <a href="{{ $converter->cal_link }}/" class="text-decoration-none">{{ $converter->cal_title }}</a>    
                                    </strong>
                                </h2>
                                @foreach($grouped_converters[$converter->cal_title] as $sub_converter)

                                <h2 class="text-20px col-span-12 md:col-span-6">
                                    <ul class="list-disc pl-4 marker:text-black marker:text-[22px]">
                                        <li class="">
                                            <a href="{{ url($sub_converter->cal_link) }}" class="bg-white py-1 text-[18px] font-medium rounded-[12px] block hover:underline hover:text-black">
                                                {{ $sub_converter->cal_title }}
                                            </a>
                                        </li>
                                    </ul>
                                </h2>
