...
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
	<div class="row">
		<p class="col-sm-12 h2">Edit Converter</p>
		<div class="col-sm-12">
            ...
            @endif
			<form method="POST" action="{{ url('admin/edit-converter/'.$page->cal_id) }}/" accept-charset="UTF-8" enctype="multipart/form-data">
				...
				<div class="col-md-6 col-md-12">
					<div class="form-group">
						...
						<select class="form-control" required name="cal_cat" id="cat_cal">
							<option disabled>Select Converter Category</option>
							<option {{$page->cal_cat=='common-converters'?'selected':''}} value="common-converters">Common Converters</option>
                            <option {{$page->cal_cat=='engineering-converters'?'selected':''}} value="engineering-converters">Engineering Converters</option>
                            <option {{$page->cal_cat=='electricity-converters'?'selected':''}} value="electricity-converters">Electricity Converters</option>
                            <option {{$page->cal_cat=='heat-converters'?'selected':''}} value="heat-converters">Heat Converters</option>
                            <option {{$page->cal_cat=='magnetism-converters'?'selected':''}} value="magnetism-converters">Magnetism Converters</option>
                            <option {{$page->cal_cat=='fluids-converters'?'selected':''}} value="fluids-converters">Fluids Converters</option>
                            <option {{$page->cal_cat=='radiology-converters'?'selected':''}} value="radiology-converters">Radiology Converters</option>
                            <option {{$page->cal_cat=='light-converters'?'selected':''}} value="light-converters">Light Converters</option>
                            <option {{$page->cal_cat=='other-converters'?'selected':''}} value="other-converters">Other Converters</option>
						</select>
					</div>
					<div class="form-group">
						<label class="h4">Converter Title</label>
						<input class='form-control' type='text' name='cal_title' id="naam" value="{{$page->cal_title}}" required placeholder="Converter Title" />
					</div>
					<div class="form-group">
				        <label class="h4">Converter URL</label>
				    ...
				    <div class="form-group">
						<label class="h4">Select Related Converter</label><br>
							...
							@php
                                if (is_array($related)) {
                                    foreach ($related as $key => $value1) {
                                        if (is_numeric($key)) {
                                            $name=explode('/', $value1);
                                            echo "<p style='margin-top:5px'>No. ".($key+1)." Related Converter</p><select class='form-control select2' name='related_cal".($key+1)."'><option value='khali'>Select Related Converter</option>";
                                            foreach ($parent as $value) {
                                                if (isset($name[3])) {
					...
					<input type="hidden" name="is_calculator" value="Converter">
					@if ($page->cal_img)	
						<div class="form-group">
							<label>Icon</label>
	...
	<script>
		...
		$(document).ready(function(){
			...
			function add_related(j){
            	var html='<p style="margin-top:5px">Add '+j+' Related Converter</p>'+
                '<select class="select2 active form-control" name="related_cal'+j+'">'+
               	'<option disabled selected>Select Related Converter</option>'+options+'</select>';
