<div x-data="{ 
    equ: @entangle('equ').live,
    examples: ['(x+4)/(2x-5)', '2x+3', '(3x-2)/(x+1)', '5x-7', '(2x+1)/(3-x)'],
    loadRandomExample() {
        let current = this.equ;
        let filtered = this.examples.filter(ex => ex !== current);
        this.equ = filtered[Math.floor(Math.random() * filtered.length)];
        $wire.detail = null;
        $wire.error = null;
    }
}">
            ...
            <div class="grid grid-cols-12 mt-3   gap-2 md:gap-4 lg:gap-4">
                ...
                <div class="col-span-12">
                    <div class="grid grid-cols-12 mt-3   gap-2 md:gap-4 lg:gap-4">
                        ...
                        <button type="button" class="flex items-center p-1" id="exampleLoadBtn" @click="loadRandomExample()">
                    ...
                    <div class="w-100 py-2">
                        <input type="text" name="equ" id="equ" class="input" x-model="equ" aria-label="input" />
                    </div>
                </div>
