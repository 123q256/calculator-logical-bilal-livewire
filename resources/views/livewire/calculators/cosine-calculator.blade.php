            ...
            <div class="lg:w-[40%] md:w-[40%] w-full mx-auto">
                <div class="grid grid-cols-12 gap-2 md:gap-4 lg:gap-4">
                        ...
                        <div class="relative w-full mt-[7px]">
                            ...
                            <div class="absolute right-6 top-1/2 -translate-y-1/2 h-full flex items-center z-[1001]">
                                ...
                                <div x-show="openUnit" @click.away="openUnit = false" x-transition x-cloak
                                    ...
                                    <p class="p-2 hover:bg-gray-100 cursor-pointer text-sm text-black" @click="unit = 'mrad'; openUnit = false">milliradian (mrad)</p>
                    ...
                    <div class="col-span-12 text-center flex justify-center">
                        <img src="{{ asset('images/sin_prop.png') }}" height="100%" width="90%" alt="Cosine Formula Image" style="object-fit: contain;" loading="lazy" decoding="async">
                    </div>
                </div>
            </div>
                            ...
                            <div class="w-full md:w-[60%] lg:w-[60%] mt-2">
                                <table class="w-full text-[16px]">
                                    @php
                                        ...
                                        $radicalVal = '';
                                        if ($angle_unit === 'deg') {
                                            $absCos = abs(floatval($cosVal));
                                            foreach ($specialValues as $key => $latex) {
                                                if (abs($absCos - floatval($key)) < 0.00001) {
                                                    $radicalVal = (floatval($cosVal) < 0 ? '-' : '') . $latex;
                                                    break;
                                                }
                                            }
                                        }
                                    ...
                                    @if(!empty($radicalVal))
                                        <tr>
                                            <td class="py-2 border-b" width="60%"><strong>cos({{ $angle }}{{ $unitLabel }})</strong></td>
                                    ...
                                    <tr>
                                        <td class="py-2 border-b" width="60%"><strong>cos({{ $angle }}{{ $unitLabel }})</strong></td>
                                        <td class="py-2 border-b">{{ $cosVal }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
