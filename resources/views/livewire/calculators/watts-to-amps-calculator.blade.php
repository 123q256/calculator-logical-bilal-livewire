                    ...
                    <div class="flex flex-wrap items-center bg-blue-100 border border-blue-500 text-center rounded-lg px-1">
                        <div class="lg:w-1/2 w-full px-2 py-1">
                            <div class="bg-white px-3 py-2 cursor-pointer rounded-md transition-colors duration-300 hover_tags hover:text-white">
                                <a href="{{ url('amps-to-watts-calculator') }}/" class="cursor-pointer text-decoration-none">
                                    <strong>{{ isset($lang['1']) ? $lang['1'] : 'Amps to Watts' }}</strong>
                                </a>
                            </div>
                        </div>
                        <div class="lg:w-1/2 w-full px-2 py-1">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 md:grid-cols-2 mt-3 gap-2">
                    ...
                    <div class="col-span-12 lg:col-span-1 px-2">
                        ...
                        <div class="relative w-full mt-[7px]">
                            <input type="number" wire:model="power" id="power" step="any" class="border border-gray-300 p-2 rounded-lg focus:ring-2 w-full" />
                            <label wire:click="toggleDropdown('power_unit')" class="absolute cursor-pointer text-sm underline right-6 top-4">{{ $power_unit }} ▾</label>
                            @if($openDropdown == 'power_unit')
                                <div class="absolute z-10 bg-white border border-gray-300 rounded-md w-auto mt-1 right-0 shadow-lg">
                                    <p class="p-2 hover:bg-gray-100 cursor-pointer" wire:click="setUnit('power_unit', 'mW')">mW</p>
                                    <p class="p-2 hover:bg-gray-100 cursor-pointer" wire:click="setUnit('power_unit', 'W')">W</p>
                                    <p class="p-2 hover:bg-gray-100 cursor-pointer" wire:click="setUnit('power_unit', 'kW')">kW</p>
                                </div>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="col-span-12 lg:col-span-1 px-2">
                            ...
                            <div class="w-full py-2">
                                <select wire:model="voltage_type" id="voltage_type" class="input">
                                    <option value="ltl">{{ $lang[12] }}</option>
                                    <option value="ltn">{{ $lang[13] }}</option>
                                </select>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($current_type != 'DC')
                        <div class="col-span-12 lg:col-span-1 px-2">
                            ...
                            <div class="w-full py-2">
                                <input type="number" step="any" max="1" wire:model="power_factor" id="power_factor" class="input" />
                            </div>
                        </div>
                    @endif
                </div>
                                    ...
                                    <table class="w-full font-s-18">
                                        <thead>
                                            <tr class="bg-[#2845F5]">
                                                ...
                                                <th class="p-2 border-b text-left text-white">{{ $lang['17'] }} Value</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
                                                <td class="py-2 border-b p-2">{{ $lang['17'] }} (amps)</td>
                                                <td class="py-2 border-b p-2"><strong class="text-blue">{{ round($detail['amps_ans'], 2) }} A</strong></td>
                                            </tr>
                                            <tr>
                                                <td class="py-2 border-b p-2">{{ $lang['17'] }} ({{ $lang['18'] }})</td>
                                                <td class="py-2 border-b p-2"><strong class="text-blue">{{ round(($detail['amps_ans'] * 1000), 2) }} mA</strong></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
