            ...
            <div class="grid grid-cols-12 mt-3   gap-2 md:gap-4 lg:gap-4">
                <div class="col-span-12 md:col-span-6 lg:col-span-6">
                    ...
                    <div class="w-full py-2">
                        <input type="text" name="fx" id="fx" class="input" wire:model.live="fx" aria-label="input"/>
                    </div>
                </div>
                <div class="col-span-12 md:col-span-6 lg:col-span-6">
                    <label for="fx1" class="labele">f'(x) {{$lang[1]}}</label>
                    <div class="w-full py-2">
                        <input type="text" name="fx1" id="fx1" class="input" wire:model.live="fx1" aria-label="input"/>
                    </div>
                </div>
                <div class="col-span-12 md:col-span-6 lg:col-span-6">
                    <label for="x0" class="labele">{{$lang['2']}} (x₀)</label>
                    <div class="w-full py-2">
                        <input type="number" step="any" name="x0" id="x0" class="input" wire:model.live="x0" aria-label="input"/>
                    </div>
                </div>
                <div class="col-span-12 md:col-span-6 lg:col-span-6">
                    <label for="iter" class="labele">{{$lang[3]}}</label>
                    <div class="w-full py-2">
                        <input type="number" name="iter" min="10" max="200" id="iter" class="input" wire:model.live="iter" aria-label="input"/>
                    </div>
                </div>
                <div class="col-span-12 md:col-span-6 lg:col-span-6">
                    <label for="round" class="labele">{{$lang['4']}}</label>
                    <div class="w-full py-2">
                        <input type="number" min="3" max="20"  step="any" name="round" id="round" class="input" wire:model.live="round" aria-label="input"/>
                    </div>
                </div>
                                ...
                                @for ($i=0; $i < count($ress)-1; $i++)
                                    @php
                                        if($i===0){
                                            ...
                                            $x_val=sigFig($ress[$i-1],$round);
                                        }
                                    @endphp
                                    <p class="mt-3">\( {{$lang['10']}} \ {{$i+1}}: \)</p>
                                    <p class="mt-3">\( f(x_{{$i}}) = f({{$x_val}}) = {{preg_replace('/x/','('.$x_val.')',$enter)}} = {{round($fxx[$i],$round)}} \)</p>
                                    <p class="mt-3">\( f'(x_{{$i}}) = f'({{$x_val}}) = {{preg_replace('/x/','('.$x_val.')',$enter1)}} = {{sigFig($fxx1[$i],$round)}} \)</p>
                                    <p class="mt-3">\( x_{{$i+1}} = x_{{$i}} - \)<span class="font_size22">\( \frac {f(x_{{$i}})} {f'(x_{{$i}})} \)</span></p>
                                    <p class="mt-3">\( x_{{$i+1}} = {{$x_val}} - \)<span class="font_size22">\( \frac {{{round($fxx[$i],$round)}}} {{{sigFig($fxx1[$i],$round)}}} \)</span></p>
