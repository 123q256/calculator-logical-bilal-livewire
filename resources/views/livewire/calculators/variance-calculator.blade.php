       ...
       <div class="lg:w-[60%] md:w-[60%] w-full mx-auto ">
            ...
            <div class="col-span-12">
                ...
                <div class="w-100 py-2">
                    <textarea wire:model.live="dataSet" id="set" class="textareaInput" aria-label="input" placeholder="e.g. 12, 23, 45, 33, 65, 54, 54
55 99 85"></textarea>
                </div>
            </div>
            <div class="col-span-12 text-center flex items-center justify-between">
                <div class="d-flex align-items-center">
                    <input wire:model.live="cal_meth" id="cal_meth" value="sample" type="radio" />
                    <label for="cal_meth" class="font-s-14 text-blue cursor-pointer pe-lg-3 px-1">{{ $lang['2'] }}</label>
                    <input wire:model.live="cal_meth" id="cal_meth1" value="population" type="radio" />
                    ...
                    <div class="w-full">
                        ...
                        <p class="w-full mt-2">
                            @if ($cal_meth==="population")
                                ...
                                \[ {{ $sdSym }}^2 = {{ ($t_n > 0) ? ($ss / $t_n) : 0 }} \]
                            @else
                                \[ {{ $sdSym }}^2 = \dfrac{SS}{n - 1} \]
                                \[ {{ $sdSym }}^2 = \dfrac{ {{ $ss }} }{ {{ ($t_n - 1) }} } \]
                                \[ {{ $sdSym }}^2 = {{ ($t_n > 1) ? ($ss / ($t_n - 1)) : 0 }} \]
                            @endif
                        </p>
                        ...
                        @if ($cal_meth==="population")
                            \[ {{ $sdSym }} = \sqrt{ {{ $sdSym }}^2} \]
                            \[ {{ $sdSym }} = \sqrt{ {{ ($t_n > 0) ? ($ss / $t_n) : 0 }} } \]
                            \[ {{ $sdSym }} = {{ round(($t_n > 0) ? sqrt($ss / $t_n) : 0, 4) }} \]
                        @else
                            \[ {{ $sdSym }} = \sqrt{ {{ $sdSym }}^2} \]
                            \[ {{ $sdSym }} = \sqrt{ {{ ($t_n > 1) ? ($ss / ($t_n - 1)) : 0 }} } \]
                            \[ {{ $sdSym }} = {{ round(($t_n > 1) ? sqrt($ss / ($t_n - 1)) : 0, 4) }} \]
                        @endif
                        </p>
