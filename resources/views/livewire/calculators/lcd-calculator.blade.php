<div>
 <form wire:submit.prevent="calculate">
            ...
            <div class="grid grid-cols-12 mt-3   gap-2 md:gap-4 lg:gap-4">
                <div class="col-span-12">
                    ...
                    <div class="w-full py-2">
                        <textarea aria-label="textarea input" id="x" name="x" wire:model.live="x" class="textareaInput"></textarea>
                    </div>
                </div>
        </div>
    </div>
    ...
    @if(isset($detail) && isset($detail['lcm']))
    <hr>
                    ...
                    <div class="w-full mt-3">
                        <div class="w-full">
                            ...
                            <div class="col-12 mt-2">
                                ...
                                <p class="mt-2"><?=$lang['9']?> LCM <?=$lang['10']?> <a href="{{ url('lcm-calculator') }}/" class="text-blue-500 underline" target="_blank">LCM Calculator</a></p>
                                <p class="mt-2"><?=$lang['11']?>, LCD (<?=$lang['3']?>) <?=$lang['12']?> <?=$detail['lcm']?></p>
                                <p class="mt-2"><?=$lang['13']?> LCD:</p>
                                <table class="w-full md:w-[80%] lg:w-[80%] overflow-auto mt-2">
                                    <?php foreach ($detail['input'] as $key => $value) {?>
                                        <tr>
                                            <td class="py-2 text-center border-b"><?=trim($value)?></td>
                                            <td class="py-2 text-center border-b">=</td>
                                            <td class="py-2 text-center border-b"><?=trim($value)?></td>
                                            <td class="py-2 text-center border-b">x</td>
                                            <td class="py-2 text-center border-b"><?=$detail['multiple'][$key]?>/<?=$detail['multiple'][$key]?></td>
                                            <td class="py-2 text-center border-b">=</td>
                                            <td class="py-2 text-center border-b"><?=$detail['upper'][$key]*$detail['multiple'][$key]?>/<?=$detail['btm'][$key]*$detail['multiple'][$key]?></td>
                                        </tr>
                                    <?php } ?>
                                </table>
                            </div>
