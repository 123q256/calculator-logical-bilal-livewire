    ...
    <div class="w-full mx-auto p-4 lg:p-8 md:p-8 input_form rounded-lg space-y-6 my-3">
        ...
        <div class="lg:w-[60%] md:w-[60%] w-full mx-auto">
            <div class="grid grid-cols-12 mt-3 gap-2 md:gap-4 lg:gap-4">
                ...
                <div class="col-span-12">
                    <div class="grid grid-cols-12 mt-3 gap-2 md:gap-4 lg:gap-4">
                        <div class="col-span-12 md:col-span-6 lg:col-span-6 flex items-center">
                            <label for="dividend" class="label" id="heading2">{{ $lang[1] ?? 'Dividend' }} y = f(x) =</label>
                        </div>
                        <div class="col-span-12 md:col-span-6 lg:col-span-6 flex justify-end items-center">
                            <button type="button" class="flex items-center p-1" id="exampleLoadBtn" @click="
                                const eq = [
                                    '3x^3 + x^2 - 4x + 1',
                                    'x^3 + x^2 + x + 1',
                                    '3x^3 - 5x^2 + 10x - 3',
                                ...
                                dividend = eq[Math.floor(Math.random() * eq.length)];
                            ">
                ...
                <div class="col-span-12">
                    <label for="divisor" class="font-s-14 text-blue">{{ $lang['2'] ?? 'Divisor' }}</label>
                    <div class="w-full py-2">
                                    ...
                                    \( 
                                        ...
                                        <?php
                                         for($i=1; $i < $loop_len; $i++) {
                                             $and.="&";
                                             echo '\\\\' . substr($and, '1') . '-\\\\\phantom{\\enclose{longdiv}{}}' . substr($and, '1') . preg_replace('/(\\s\\+\\s)/',' & + ',(preg_replace('/(\\s-\\s)/',' & - ',$multiplies[$i]))) . '\\\\\\hline\\phantom{\\enclose{longdiv}{}}' . $and . preg_replace('/(\\s\\+\\s)/',' & + ',(preg_replace('/(\\s-\\s)/',' & - ',$steps[$i])));
                                         }
                                         ?>
    ...
    @push('calculatorJS')
        <script>
                function loadMathJax() {
                    ...
                    mathJaxConfigScript.textContent = 'MathJax.Hub.Config({"HTML-CSS": {linebreaks: { automatic: true }},"CommonHTML": {linebreaks: { automatic: true }}}); function MJrerender() { MathJax.Hub.Queue(["Typeset", MathJax.Hub]); }';
                ...
                window.addEventListener('math-updated', function () {
                    setTimeout(function () {
