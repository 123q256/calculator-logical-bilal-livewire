                    ...
                    <div class="rounded-lg flex items-center justify-center">
                        <div class="w-full mt-3">
                            <div class="w-full">
                                    ...
                                    <table class="w-full" style="border-collapse: collapse">
                                        ...
                                        @foreach($x_data as $key => $value)
                                            <tr class="bg-white">
                                                ...
                                                <td class="p-2 text-center border">{{ $y_data[$key] }}</td>
                                            </tr>
                                        @endforeach
                                    </table>
                                </div>
                                ...
                                <p class="w-full mt-2 overflow-auto">$$ SS_{XX} = \sum^n_{i-1}X_i^2 - \dfrac{1}{n} \left(\sum^n_{i-1}X_i \right)^2 = {{ $xi_sum }} - \dfrac{1}{ {{ $n }} } ({{ $x_sum }})^2 = {{ sigFig($ss_xx, 5) }} $$</p>
                                
                                <p class="w-full mt-2 overflow-auto">$$ SS_{YY} = \sum^n_{i-1}Y_i^2 - \dfrac{1}{n} \left(\sum^n_{i-1}Y_i \right)^2 = {{ $yi_sum }} - \dfrac{1}{ {{ $n }} } ({{ $y_sum }})^2 = {{ sigFig($ss_yy, 5) }} $$</p>
                                
                                <p class="w-full mt-2 overflow-auto">$$ SS_{XY} = \sum^n_{i-1}X_iY_i - \dfrac{1}{n} \left(\sum^n_{i-1}X_i \right) \left(\sum^n_{i-1}Y_i \right) = {{ $xy_sum }} - \dfrac{1}{ {{ $n }} } ({{ $x_sum }}) ({{ $y_sum }}) = {{ sigFig($ss_xy, 5) }} $$</p>
                    
                                <p class="w-full mt-3">{{ $lang['satement4'] }}:</p>
                    
                                <p class="w-full mt-2 overflow-auto">$$ \hat{\beta}_1 = \dfrac{SS_{XY}}{SS_{XX}} = \dfrac{ {{ sigFig($ss_xy, 5) }} }{ {{ sigFig($ss_xx, 5) }} } = {{ sigFig($beta_1, 5) }} $$</p>
                    
                                <p class="w-full mt-2 overflow-auto">$$ \hat{\beta}_0 = \bar{Y} - \hat{\beta}_1 \times \bar{X} = {{ sigFig(($y_sum / $n), 5) }} - {{ sigFig($beta_1, 5) }} \times {{ sigFig(($x_sum / $n), 5) }} = {{ sigFig($beta_0, 5) }} $$</p>
                    
                                <p class="w-full mt-3">{{ $lang['satement5'] }}:</p>
                                <p class="w-full mt-2 overflow-auto">$$ \hat{Y} = {{ sigFig($beta_0, 5) }} + {{ sigFig($beta_1, 5) }}X $$</p>
                    
                                <p class="w-full mt-3 text-blue font-s-18">{{ $lang['satement6'] }} $$ \hat{Y} = {{ sigFig($beta_0, 5) }} + {{ sigFig($beta_1, 5) }}X $$</p>
                                ...
                                <div class="grid grid-cols-1 mt-2 overflow-auto">
                                    <table class="w-full" style="border-collapse: collapse">
                                        <tr class="bg-gray-100">
                                            <td class="p-2 border text-center text-blue"><strong>Obs.</strong></td>
                                            <td class="p-2 border text-center text-blue"><strong>X</strong></td>
                                            <td class="p-2 border text-center text-blue"><strong>Y</strong></td>
                                            <td class="p-2 border text-center text-blue"><strong>Predicted Values $$ (\hat{Y}) $$</strong></td>
                                            <td class="p-2 border text-center text-blue"><strong>Residuals $$ (Y - \hat{Y}) $$</strong></td>
                                        </tr>
                                        @foreach($x_data as $key => $xi)
                                            @php $yi = $y_data[$key]; @endphp
                                            <tr class="bg-white">
                                ...
                                <p class="w-full my-6 text-blue font-s-18">{{ $lang['satement8'] }}:</p>
                                
                                <div class="w-full mt-4" x-data="{
                                    initChart() {
                                        if(window.myResidualPlotChart) {
                                            window.myResidualPlotChart.destroy();
                                        }
                                        const ctx = document.getElementById('myChart');
                                        if(!ctx) return;

                                        const xyValues = [
                                            @foreach($y_bar as $key => $value)
                                                {x: {{ $key + 1 }}, y: {{ $yy_bar[$key] }} },
                                            @endforeach
                                        ];

                                        window.myResidualPlotChart = new Chart(ctx, {
                                            type: 'line',
                                            data: {
                                                datasets: [{
                                                    pointRadius: 4,
