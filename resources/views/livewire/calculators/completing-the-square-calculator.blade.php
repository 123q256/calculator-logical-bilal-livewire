                ...
                <div class="w-full mt-3">
                    <div class="w-full">
                        <div class="w-full text-[16px]">
                            ...
                            <p class="mt-2">
                                \( Standard \ Form: <?= $A ?>x^2 <?= $detail['B'] < 0 ? $detail['B'] : ' + ' . $detail['B'] ?>x
                                <?= $detail['C'] < 0 ? $detail['C'] : ' + ' . $detail['C'] ?> = 0 \)
                            </p>
                            <p class="mt-2">
                                \( a = <?= $A ?>, \ b = <?= $detail['B'] ?>, \ c = <?= $detail['C'] ?> \)
                            </p>
                            <?php if ($A != 1) { ?>
                            <p class="mt-2">
                                \( a \ne 1 \ \text{<?= $lang['divide'] ?>} \ <?= $A ?> \)
            
                            </p>
                            <p class="mt-2">
                                \( { x^2}<?= $detail['B'] < 0 ? ' - ' : ' + ' ?> \frac{
                                <?= $detail['B'] < 0 ? $detail['B'] * -1 : $detail['B'] ?> x}{ <?= $A ?> } =
                                <?= $detail['C'] < 0 ? ' ' : ' - ' ?> \frac{
                                <?= $detail['C'] < 0 ? $detail['C'] * -1 : $detail['C'] ?>}{ <?= $A ?> } \)
                            </p>
                            <p class="mt-2">
                            ...
                            <p class="mt-2">
                                \( { x^2}<?= $detail['B'] < 0 ? ' - ' : ' + ' ?> \frac{
                                <?= $detail['B'] < 0 ? $detail['B'] * -1 : $detail['B'] ?> x}{ <?= $A ?> } + \frac{
                                <?= pow($detail['B'], 2) ?> }{ <?= pow($A * 2, 2) ?> } = <?= $detail['C'] < 0 ? ' ' : ' - ' ?>
                                \frac{ <?= $detail['C'] < 0 ? $detail['C'] * -1 : $detail['C'] ?>}{ <?= $A ?> } + \frac{
                                <?= pow($detail['B'], 2) ?> }{ <?= pow($A * 2, 2) ?> } \)
            
                            </p>
                            <p class="mt-2">
                                \( ({ x }<?= $detail['B'] < 0 ? ' - ' : ' + ' ?> \frac{
                                <?= $detail['B'] < 0 ? $detail['B'] * -1 : $detail['B'] ?>}{ <?= $A * 2 ?> })^2 =
                                <?= $detail['C'] < 0 ? ' ' : ' - ' ?> \frac{
                                <?= $detail['C'] < 0 ? $detail['C'] * -1 : $detail['C'] ?>}{ <?= $A ?> } + \frac{
                                <?= pow($detail['B'], 2) ?> }{ <?= pow($A * 2, 2) ?> } \)
                            ...
                            <p class="mt-2">
                                ...
                                \( ({ x }<?= $detail['B'] < 0 ? ' - ' : ' + ' ?> \frac{
                                <?= $detail['B'] < 0 ? $detail['B'] * -1 : $detail['B'] ?>}{ <?= $A * 2 ?> })^2 =
                                <?= $detail['C'] < 0 ? ' ' : ' - ' ?> \frac{ <?= $right_side ?> }{ <?= pow($A * 2, 2) ?> } \)
                            </p>
                            <p class="mt-2">
                                <?php if ($right_side < 0) {
                                ...
                                <?php if ($right_side != 0) { ?>
                                \( { x }<?= $detail['B'] < 0 ? ' - ' : ' + ' ?> \frac{
                                <?= $detail['B'] < 0 ? $detail['B'] * -1 : $detail['B'] ?>}{ <?= $A * 2 ?> } = \pm \sqrt{
                                <?= $right_side < 0 ? ' - ' : ' ' ?> \frac{ <?= $right_side ?> }{ <?= pow($A * 2, 2) ?> }}
                                <?= @$i ?> \) </p>
                            <p class="mt-2">
                                \( { x } = <?= $detail['B'] < 0 ? ' + ' : ' - ' ?> \frac{
                                <?= $detail['B'] < 0 ? $detail['B'] * -1 : $detail['B'] ?>}{ <?= $A * 2 ?> } \pm \sqrt{
                            ...
                            <p class="mt-2"> \( { x₁ } = <?= $detail['B'] < 0 ? ' + ' : ' - ' ?> \frac{
                                <?= $detail['B'] < 0 ? $detail['B'] * -1 : $detail['B'] ?>}{ <?= $A * 2 ?> } + \sqrt{
                                <?= $right_side < 0 ? ' - ' : ' ' ?> \frac{ <?= $right_side ?> }{ <?= pow($A * 2, 2) ?> }}
                                <?= @$i ?> , { x₁ = <?= $detail['x1'] ?> } <?= @$i ?> \)</p>
                            <p class="mt-2">\( { x₂ } = <?= $detail['B'] < 0 ? ' + ' : ' - ' ?> \frac{
                                <?= $detail['B'] < 0 ? $detail['B'] * -1 : $detail['B'] ?>}{ <?= $A * 2 ?> } - \sqrt{
                            ...
                            <p class="mt-2">
                                \( { x } = <?= $detail['B'] < 0 ? ' + ' : ' - ' ?> \frac{
                                <?= $detail['B'] < 0 ? $detail['B'] * -1 : $detail['B'] ?>}{ <?= $A * 2 ?> } , { x =
                                <?= $detail['x1'] ?> }\)
                                <?php } ?>
                            </p>
                            <p class="mt-2">
