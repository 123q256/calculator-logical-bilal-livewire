            ...
            <div class="grid grid-cols-12 mt-3   gap-2 md:gap-4 lg:gap-4">
                ...
                <div class="col-span-12">
                    ...
                    <div class="w-full py-2">
                        <input type="text" name="x" id="x" class="input" wire:model.live="x" aria-label="input"/>
                    </div>
                </div>
                ...
                <div class="col-span-6">
                    ...
                    <div class="w-full py-2">
                        <input type="number" step="any" name="x2" id="x2" min="1" class="input" wire:model.live="x2" aria-label="input"/>
                    </div>
                </div>
                ...
                <div class="w-full mt-3">
                    <div class="w-full">
                        ...
                        <div class="w-full text-[16px]">
                            ...
                            <p class="mt-3">\( f(x) = {{$detail['eq']}} \) {{$lang['4']}} = \( [{{$x1}} , {{$x2}}] \)</p>
                            <p class="mt-3">{{$lang['7']}}</p>
                            <p class="mt-3">\( \frac{f(b) - f(a)}{b - a} \)</p>
                            <p class="mt-3">{{$lang['8']}}</p>
                            <p class="mt-3">\( a = {{$x2}}, b = {{$x1}}, f(x) = {{(preg_match('/frac/',$detail['eq']))?'\) \(\space '.$detail['eq'].'\) \(':$detail['eq']}}\)</p>
                            <p class="mt-3">Step 1: Evaluate the Function for a and b</p>
                            <p class="mt-3">\(f(a) = f({{$x1}}) = ({{preg_replace('/x/','('.$x1.')',$detail['eq'])}}) = {{$detail['fx1']}}\)</p>
                            <p class="mt-3">\(f(b) = f({{$x2}}) = ({{preg_replace('/x/','('.$x2.')',$detail['eq'])}}) = {{$detail['fx2']}}\)</p>
                            <p class="mt-3">Step 2: Use the Average Rate of Change Formula</p>
                            <p class="mt-3">\( \frac{f(b) - f(a)}{b - a} = \frac{({{$detail['fx2']}}) - ({{$detail['fx1']}})}{{{$x2.'-'.$x1}}} \)</p>
                            <p class="mt-3">\( \frac{f(b) - f(a)}{b - a} = \frac{{{$detail['fx2']-$detail['fx1']}}}{{{$x2-$x1}}} \)</p>
