<div>
  <form wire:submit.prevent="calculate">
    <div class="w-full mx-auto p-4 lg:p-8 md:p-8 input_form rounded-lg  space-y-6 my-3" x-data="{ units: @entangle('units') }">
        @if (isset($error))
                ...
                <div class="col-span-12 md:col-span-9 lg:col-span-9">
                    ...
                    <div class="col-span-10">
                        ...
                        <div class="w-full py-2">
                            <select wire:model.live="units" class="input" id="conversion" aria-label="select">
                                <option value="A Sequence">{{$lang[3]}}</option>
                                <option value="One Number">{{$lang[4]}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-span-10">
                        <div class="items-center justify-center mt-0 mt-lg-2 py-2  flex " x-show="units === 'A Sequence'" id="two">
                            <div>F<sub>n</sub>&nbsp;for&nbsp;n&nbsp;=&nbsp;</div>
                            <div>
                                <input type="number" step="any" wire:model.live="first_term" id="first_term" class="input" aria-label="input" />
                            </div>
                            <div class="px-2">to</div>
                            <div>
                                <input type="number" step="any" wire:model.live="second_term" id="second_term" class="input" aria-label="input" />
                            </div>
                        </div>
                        <div class="items-center justify-center mt-0 mt-lg-2 py-2  flex " x-show="units === 'One Number'" style="display: none;" id="one">
                            <div>F<sub>n</sub> for n = </div>
                            <div>
                        ...
                        @if($units === "A Sequence")
                            <div class="w-full md:w-[60%] lg:w-[60%] overflow-auto mt-2">
                                <table class="w-full text-[18px]">
                                    <tr>
                                        <td class="py-2 border-b" colspan="2"><strong>F<sub class="font-s-14">{{$first_term}}</sub> to F<sub class="font-s-14">{{$second_term}}</sub> = </strong> {{implode(" , ", $detail['fibonacci_sequence'])}}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    @foreach($detail['fibonacci_sequence'] as $key => $value)
                                        <tr>
                                            <td class="py-2 border-b" width="60%">{{ $key + $first_term }}</td>
                            ...
                            <div class="w-full text-center font-s-20">
                                <p class="my-3">
                                    <strong class="bg-sky px-3 py-2 text-[32px] rounded-lg text-blue">
                                        F<sub class="font-s-14">{{ $n }}</sub> = {{ safe_round($detail['answer']) }}
                                    </strong>
                                </p>
                            </div>
                        @endif
