    ...
    <div class="w-full mx-auto p-4 lg:p-8 md:p-8 input_form rounded-lg  space-y-6 my-3">
       ...
       <div class="lg:w-[60%] md:w-[60%] w-full mx-auto ">
         ...
         <div class="col-span-9">
                <label for="EnterEq" class="text-[14px]">{{ $lang['1'] ?? 'Enter Equation' }}:</label>
            ...
            <div class="col-span-3">
                <label for="with" class="text-[14px]">{{ $lang['2'] ?? 'with respect to' }}:</label>
                ...
                <div class="w-full mt-3">
                    <div class="w-full">
                        ...
                        <div class="w-full md:w-[60%] lg:w-[60%] mt-2 px-2 py-1">
                            ...
                            <p class="mt-3 text-center"><strong>{{ $lang[3] ?? 'Radius of Convergence formula:' }}</strong></p>
                            <p class="bg-[#ffffff] border rounded-lg px-3 py-2 mt-2 text-[18px] overflow-auto text-center">\(\left|<?=$detail['ans']?>\right| \le 1\)</p>
                            <p class="my-3"><strong>{{ $lang[4] ?? 'Step by step solution:' }}</strong></p>
                            <p class="my-3">{{ $lang[5] ?? 'Write the series in terms of sum:' }}</p>
                            <p class="my-3 font-s-20">\( \sum_{n=1}^\infty<?=$detail['enter']?>\)</p>
                            <p class="my-3">{{ $lang[6] ?? 'Use the Ratio Test to find the limit of the series:' }}</p>
                            <p class="my-3">{{ $lang[7] ?? 'The series converges if' }} L < 1:</p>
                            ...
                            <p class="my-3">{{ $lang[8] ?? 'Simplify and take the limit:' }}:</p>
                            <p class="my-3">\( L=\left|<?=$detail['ans']?>\right|\)</p>
                            <p class="my-3">{{ $lang[9] ?? 'Set the limit L < 1 to find the interval of convergence:' }}</p>
