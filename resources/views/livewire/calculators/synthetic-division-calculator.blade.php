<div x-data="{
    dividend: @entangle('dividend').live,
    divisor: @entangle('divisor').live,
    init() {
        this.updatePreview();
        this.$watch('dividend', () => this.updatePreview());
        this.$watch('divisor', () => this.updatePreview());
    },
    updatePreview() {
        this.$nextTick(() => {
            let eq = '\\frac{' + (this.dividend || '') + '}{' + (this.divisor || '') + '}';
            let previewEl = document.getElementById('equation_preview');
            if (previewEl && typeof katex !== 'undefined') {
                try {
                    katex.render(eq, previewEl, {
                        throwOnError: false,
                        displayMode: true
                    });
                } catch (e) {
        ...
        <div class="lg:w-[60%] md:w-[60%] w-full mx-auto">
            <div class="grid grid-cols-12 mt-3 gap-2 md:gap-4 lg:gap-4">
                <div class="col-span-12">
                    <label for="dividend" class="font-s-14 text-blue">{{ $lang[1] ?? 'Dividend' }}</label>
                    <div class="w-full py-2">
                        <input type="text" name="dividend" id="dividend" class="input" x-model="dividend" aria-label="input"/>
                    </div>
                </div>

                <div class="col-span-12">
                    <label for="divisor" class="font-s-14 text-blue">{{ $lang[2] ?? 'Divisor' }} (ax + b)</label>
                    <div class="w-full py-2">
                        <input type="text" name="divisor" id="divisor" class="input" x-model="divisor" aria-label="input"/>
                    </div>
                </div>

                <div class="col-span-12">
                    <label for="equation_preview" class="font-s-14 text-blue">Equation Preview</label>
                    <p class="col-12 mt-0 mt-lg-2 pt-2 radius_10 bg-sky text-center mx-auto overflow-auto radius-5 border" id="equation_preview" wire:ignore></p>
                </div>
            </div>
        </div>
                    ...
                    <div class="w-full mt-3">
                        <div class="w-full">
                            ...
                            <div class="col-12 text-[16px] {{ $device == 'mobile' ? 'overflow-auto' : '' }}">
                                ...
                                <div class="bg-[#F6FAFC] border rounded-lg px-3 py-2 my-4 overflow-auto synth_steps">
                                    <p class="mt-3">\( \dfrac{ {{ $dividend }} }{ {{ $divisor }} } \)</p>
                                    <p class="mt-3">{{ $lang[6] ?? 'Write the dividend coefficients:' }}</p>
                                    <p class="mt-3">
                                        \(
                                            @php
                                    ...
                                    <p class="mt-3">{{ $lang[7] ?? 'Set divisor to 0 and solve:' }}</p>
                                    ...
                                    <p class="mt-3">{{ $lang[8] ?? 'Set up the synthetic division:' }}</p>
                                    <p class="mt-4">
                                        \(
                                            \begin{array}{c|rrrrr}&
                                            @php
                                    ...
                                    <p class="mt-3">{{ $lang[9] ?? 'Bring down the first coefficient:' }}</p>
                                    <p class="mt-4">
                                        \( 
                                    ...
                                    @for ($i = 0; $i < $outer_len; $i++)
                                        ...
                                        @endphp
                                        @if (($i % 2) === 0)
                                            <p class="mt-3">{{ $lang[10] ?? 'Multiply by divisor:' }}</p>
                                        ...
                                        @else
                                            <p class="mt-3">{{ $lang[11] ?? 'Add the values:' }}</p>
                                    ...
                                    <p class="mt-4">\( \text{ {{ $lang[12] ?? 'Quotient:' }} } \space { {{ $quot }} }, \space \text{ {{ $lang[13] ?? 'Remainder:' }} } \space { {{ $rmnd }} } \)</p>
                                    <p class="mt-3">{{ $lang[14] ?? 'Result equation:' }}:</p>
